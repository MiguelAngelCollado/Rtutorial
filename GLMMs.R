...
## File/Open File and search for it
load("~/Desktop/Tesis/R/Tutoriales y scripts/Scripts/GLMs.RData")
head(dragons)
...
#proceeding - you can use scale() to do that:
...
## From the above plots it looks like our mountain ranges vary both in the dragon body
#length and in their test scores. This confirms that our observations from within 
#each of the ranges aren't independent. We can't ignore that.
...
## We could run many separate analyses and fit a regression 
#for each of the mountain ranges.
...
## We want to use all the data, but account for the data coming 
...
## now body length is not significant but let’s think about what we are doing here
#for a second. The above model is estimating the difference in test scores between 
#the mountain ranges, we can see all of them in the model output returned 
#by summary(). But we are not interested in that, we just want to know whether body
#length affects test scores and we want to simply control for the variation 
#coming from mountain ranges.

#This is what we refer to as “random factors” and so we arrive at 
